...
int main08()
{
	...
	}
	{
		...
		int width, height, nrChannels;
		stbi_set_flip_vertically_on_load(true);
		unsigned char *data = stbi_load("res/image/awesomeface.png", &width, &height, &nrChannels, 0);
		if (data)
		{
...
void main()
{
    FragColor = mix(texture(tex1,fUV),texture(tex2,fUV),0.2);
}
)" };
